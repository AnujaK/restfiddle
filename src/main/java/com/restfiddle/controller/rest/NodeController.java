...
public class NodeController {
	...
	// Note : Creating a node requires parentId. Project-node is the root node
	// and it is created during project creation.
	@RequestMapping(value = "/api/nodes/{parentId}/children", method = RequestMethod.POST, headers = "Accept=application/json")
	public @ResponseBody BaseNode create(
		...
		if (nodeDTO.getConversationDTO() != null
				&& nodeDTO.getConversationDTO().getId() != null) {
			Conversation conversation = conversationRepository.findOne(nodeDTO
					.getConversationDTO().getId());
...
//		return deleted;
	...
	public @ResponseBody List<BaseNode> findAll() {
	...
	public @ResponseBody BaseNode findById(@PathVariable("id") Long id) {
	...
	public @ResponseBody List<BaseNode> getChildren(
			@PathVariable("parentId") Long parentId) {
	...
	public @ResponseBody BaseNode update(@PathVariable("id") Long id,
			@RequestBody NodeDTO updated) {
	...
	// Get tree-structure for a project. Id parameter is the project-reference
	// node-id.
	@RequestMapping(value = "/api/nodes/{id}/tree", method = RequestMethod.GET)
	public @ResponseBody TreeNode getProjectTree(@PathVariable("id") Long id) {
		// Note : There must be a better way of doing it. This method is written
		// in a hurry.
		...
		List<BaseNode> listOfNodes = nodeRepository
				.findNodesFromAProject(projectId);
		...
		for (BaseNode baseNode : listOfNodes) {
			...
			treeNode = TreeNodeBuilder.createTreeNode(nodeId,
					baseNode.getName(), baseNode.getNodeType(),
					baseNode.getStarred());
			...
			if (NodeType.PROJECT.name().equals(baseNode.getNodeType())) {
				...
				if (parentTreeNode == null) {
					parentTreeNode = TreeNodeBuilder.createTreeNode(parentId,
							baseParentNode.getName(),
							baseParentNode.getNodeType(),
	...
	public @ResponseBody List<BaseNode> findStarredNodes() {
		logger.debug("Finding starred nodes.");

		List<BaseNode> starredNodes = nodeRepository
				.findStarredNodes(Boolean.TRUE);
		return starredNodes;
	}
}
